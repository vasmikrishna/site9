...
### Fixed — Account takeover via the "Make your own website" flow (security)
- **Anyone could claim an existing account's sites by typing its email.** `POST /api/onboarding/create` never checked whether the email already had an account. Every signup mints a fresh tenant first, and `users` was only unique on `(email, tenant_id)`, so a second row for the same email always inserted — with a password the visitor chose. Since ownership resolves by **email** across all matching rows (`lib/sites.ts`), signing in with that new password returned every site already on that email. No password verification anywhere in the path.
- **Fix.** `/api/onboarding/create` now resolves the email up front (reusing `getAccountUsers`): a known email is rejected with `409 {field:"email"}` unless the caller is signed in as that account, in which case the site is attached to the existing account with **no second user row**. `/start` keeps the user on step 2 for an email conflict (it previously misreported it as a taken subdomain) and offers a sign-in link.
- **Ownership is now recorded at creation.** The route never set `tenants.owner_user_id`, leaving every new site orphaned and reliant on the legacy email backfill in `lib/sites.ts` — which hands an orphan to `ids[0]` of an *unordered* select. It is now set on insert.
- **`/api/auth/register` duplicate check was bypassable.** Its `.maybeSingle()` errors when more than one row matches and only `data` was destructured, so the error was swallowed and the 409 skipped — precisely for the emails that already had duplicates. Now `.limit(1)` with the lookup error handled.
- **Migration `022_one_user_row_per_email.sql` (written, not yet applied).** Folds duplicate rows onto the site-owning row (preserving the password so both Google and password sign-in survive), re-points all FKs, then adds `unique (lower(email))` and drops the superseded `users_email_tenant_unique`. Closes the residual race where two concurrent signups on the same new email both pass the app-level check.
