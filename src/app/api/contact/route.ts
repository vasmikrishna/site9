...
export async function POST(req: Request) {
  try {
    ...
    // 2. Email forward via Resend (if configured)
    if (process.env.RESEND_API_KEY) {
      try {
        ...
        if (!resp.ok) {
          console.error("[contact] Resend error:", await resp.text())
        }
      } catch (err) {
        console.error("[contact] Email forward failed:", err)
      }
    } else {
      console.log("[contact] new enquiry (no email — set RESEND_API_KEY to enable forwarding):", { name, email, service })
    }
